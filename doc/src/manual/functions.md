...
実際に、自分でループを書いたほうがよいこともよくあります([パフォーマンス ティップス](@ref man-performance-tips)参照)。それでも、ベクトル化は便利なので、Juliaでは *すべての* 関数 `f` に対して、配列（その他のコレクション)の各要素に適用する `f.(A)` という構文が用意されています。
