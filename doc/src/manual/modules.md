...
また、現在のプロセスにすでにロードされている依存を考慮に入れ、ファイルが変更や削除されても、実行中のシステムとプリコンパイル・キャッシュとの間の非互換性を避けるために、それらのモジュールを再コンパイルしません。
...
-->
```
しかし、モジュールをプリコンパイルしても動作するようにするには、**実行時**の初期化処理と、**コンパイル時**の処理をモジュール内で明確に分離するように変更する必要がでてくるかもしれません。
こういう用途のために、Juliaには`__init__()`関数が備わっています。
モジュール内で`__init__()`関数を定義すると、実行時に行わなければならない初期化処理を実行できます。
...
`__init__()`関数はモジュールがプロセスにロードされた後で呼び出されます。
...
特にモジュール内に、`function __init__()`が定義されている場合は、`__init__()`は（例えば`import`、`using`、`require`によって）モジュールがロードされた直後の、初めての実行時にJuliaから呼び出されます
（すなわち、モジュール内のすべての文が実行されたあとに、たった一度だけ`__init__`は呼び出されます）。
...
-->
```
`__init__`の2つの典型的な用途は、外部Cライブラリの実行時の初期化関数を呼び出すことと、外部ライブラリが返すポインタを保持するグローバル定数を初期化することです。
...
ポインタのアドレスが実行ごとに変わるため、この定数は実行時（コンパイル時ではなく）に初期化する必要があります。
これは、モジュール内で次のような`__init__`関数を定義すれば、実行できます。
...
これは動的言語を使用する利点の1つです。
しかし、これをグローバルスコープにおける定数にすれば、コンパイラが型を認識し、より最適化されたコードが生成することを保証できます。
...
ただし、他のkeyの型、例えば`Function`や`DataType`や汎化的なユーザー定義型など`hash`メソッドを直接定義していない型のの場合、副次的な`hash`メソッドは（`object_id`を介する）オブジェクトのメモリアドレスに依存するので、実行ごとに変更される可能性があります。
keyの型がこれらのいずれかの場合や、わからない場合は、安全のために`__init__`関数の中からこの辞書を初期化することができます。
...
Juliaがコンパイルされたコードを生成するスタンドアロンインタプリタではなく、任意のJuliaコードを実行できるコンパイラであることが、このプリコンパイルというモードでは、はっきりわかることがよくあります。
...
その他の既知の潜在的な障害シナリオには、
    ...
    1.グローバルカウンタ（たとえば、オブジェクトを一意に識別しようとする場合）
    次のコードスニペットを考えてみましょう。
...
-->
```
このコードはすべてのインスタンスに一意のIDを与えることを意図していましたが、カウンタ値はコンパイルの最後に記録されます。この逐次的にコンパイルした後にモジュールを使うとすべて、同じカウンタ値から開始されます。

`object_id`（メモリポインタをハッシュすることによって動作する）にも同様の問題があるので注意してください（下記の`Dict`の用法を参照)。

代替案として [`current_module()`](@ref)と現在の`counter`値の両方を保存する方法がありますが、グローバル状態に依存しないようにコードを再設計するほうがよい場合があります。
...
注意すべき他のいくつかの点は次のとおりです。
    ...
    3.ファイルシステムがコンパイル時と実行時の間で変更されないことを前提としているものに注意する（例：[`@__FILE__`](@ref)/`source_path()`や`BinDeps @checked_lib`マクロ。)
    これは避けられないこともありますが、可能であれば、コンパイル時にリソースをモジュールにコピーして実行時にリソースを見つける必要をなくすのはよい手法です。
    4.`WeakRef` オブジェクトとファイナライザは現在、シリアライザによって正しく処理されていません（これは、今後のリリースで修正される予定です）。
    5.次のような内部のメタデータ・オブジェクトのインスタンスへの参照を取得するのは避けるのが最善策です。
...
`--compilecache=no`でJuliaを起動すると、モジュールやモジュールの依存をロードするときに、コンパイル時のキャッシュで直列化されているモジュールが無視されます。
`Base.compilecache()`は依然として手動で呼び出すことができ、モジュールの`__precompile__()`ディレクティブを尊重します。
