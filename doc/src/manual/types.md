...
=======
これが必要なのは、Juliaは、使う関数のメソッドを多重ディスパッチによって選択するからです。
>>>>>>> 806cef1cc524da8a358697feb3e65c9e2cf5ce84
...
複合型のインスタンスがどのように作成されるかについては、もっと多くのことが言うべきことがありますが、その議論は[パラメトリック型](@ref) と [Methods](@ref)との両方に左右され、十分重要なので、独自のセクションで解説しています[Constructors](@ref man-constructors)。
...
まず、パラメトリック複合型、パラメトリック抽象型、最後にパラメトリック原始型の順に説明します。[訳注2]_
...
[訳注2]:Juliaはver0.5とver0.6の間で型に関する用語を大きく変えており、この訳では`bit type`にver0.6の`primitive type`の訳である原始型を当てています。