...
型注釈を加えると、一般的には、システムのパフォーマンスと堅牢性が向上します。
そして、おそらくなにか直観に反するでしょうが、非常に単純化されることもよくあります。
...
ジェネリック型はパラメータ化できて、型の階層関係は、[明示的に宣言](https://en.wikipedia.org/wiki/Nominal_type_system)するのであって、 
...
振る舞いを継承できるほうが、構造を継承できるよりも重要であり、共に継承しようとすると、従来のオブジェクト指向言語では、深刻な困難が生じることが判明しています。
...
```
  ...
  * 抽象型と具象型の両方とも、他の型によってパラメータ化できます。また、シンボル、[`isbits()`](@ref) が真になる任意の型をもつ値（本質的に数やブール値のような、他のオブジェクトへのポインタを持たないC言語の型や構造体のようなものに格納されるもの）、これらのタプルなどによってパラメータ化することもできます。参照や制限をする必要がない場合は、パラメータは省略しても構いません。
...
すべて具象型はファイナルであり、その実装が他の具象型の実装とはならないことを思い出してください。
...
つまり、C言語のような静的型付き言語の型宣言のように、変数が常に指定した型を持つという宣言になります。
...
-->
```
この機能は、変数の代入のいずれかが、想定外の型の変更をおこす時にありうる、パフォーマンスの「落とし穴」を回避するのに役立ちます。
...
そして、抽象型は具象型の集合を記述しているといえます。抽象型に子孫となる具象型を関連づけて集合と考えるのです。
...
あるコードが意味をなす、ということはよくあることで、たとえば、関数の引数を特定の種類の整数にだけ指定していても、本当はその整数の**種類**なんか関係ない場合です。
